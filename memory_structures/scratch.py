...
from utils import DEFAULT_WALLET_BALANCE
...
class Scratch: 
  def __init__(self, f_saved): 
    ...
    # ECONOMY FIELDS
    # Simple wallet balance for the persona (integer currency)
    self.wallet = None
    # Simple inventory mapping (item name -> count)
    self.inventory = dict()
    # Whether an economy side-effect (e.g., purchase) has been applied for
    # the current action; prevents duplicate purchases every step while
    # lingering at destination.
    self.act_economy_done = False

    if check_if_file_exists(f_saved): 
      # If we have a bootstrap file, load that here. Ensure UTF-8 for non-ASCII names/content.
      ...
      self.wallet = scratch_load.get("wallet")
      self.inventory = scratch_load.get("inventory", {})
      self.act_economy_done = scratch_load.get("act_economy_done", False)
      if self.wallet is None:
        self.wallet = DEFAULT_WALLET_BALANCE
  ...
  def save(self, out_json):
    ...
    # Economy fields
    scratch["wallet"] = self.wallet
    scratch["inventory"] = self.inventory
    scratch["act_economy_done"] = self.act_economy_done
  ...
  def add_new_action(self, 
    ...
    self.act_path_set = False
    # Reset per-action economy side-effect flag
    self.act_economy_done = False
