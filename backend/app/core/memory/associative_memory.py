...
class AssociativeMemory: 
  def __init__(self, f_saved): 
    ...
    if os.path.exists(f_saved + "/embeddings.json"):
        with open(f_saved + "/embeddings.json", encoding="utf-8") as _f:
            self.embeddings = json.load(_f)
    else:
        self.embeddings = {}

    if os.path.exists(f_saved + "/nodes.json"):
        with open(f_saved + "/nodes.json", encoding="utf-8") as _f:
    ...
    if os.path.exists(f_saved + "/kw_strength.json"):
        with open(f_saved + "/kw_strength.json", encoding="utf-8") as _f:
